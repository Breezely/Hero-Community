...
client.on('ready', () => {
  client.user.setGame('HeroMC Comunity | -help','https://www.twitch.tv/peery13');
  console.log('---------------');
...
client.on('message', async msg => {
    ...
    var p = "-"
  if(msg.content.startsWith('-' + "cmind")) {
...
client.on('message', async msg => {
      client.snek = require('snekfetch');
    var p = "-"
  if(msg.content.startsWith('-' + "isay")) {
   let args = msg.content.split(' ').slice(1).join(' ');
...
var gg = message.guild.channels.find('name', 'apply-staff')
if(!gg) return;
if(gg) {
...
client.on('message', (message) => {
  ...
  if(message.content.startsWith('-' + "1bc")) {
    ...
    if(message.author.id !== '300300385988509698') return message.channel.send('❎ » هذا الأمر مخصص لصاحب البوت فقط');
...
client.channels.get("300300385988509698").send(' ***  BOT  ***   **Join To**   ***[ ' + `${guild.name}` + ' ]***   ,   **  Owner  **  ' + ' ***[ ' + '<@' + `${guild.owner.user.id}` + '>' + ' ]***  **|**  ***[ ' + '<' + `${guild.owner.user.username}` + '>' + ' ]***')
});

client.on("guildDelete", guild => {
client.channels.get("504707359751536640").send(' ***  BOT  ***   **Leave From**   ***[ ' + `${guild.name}` + ' ]***   ,   **  Owner  **  ' + ' ***[ ' + '<@' + `${guild.owner.user.id}` + '>' + ' ]***  **|**  ***[ ' + '<' + `${guild.owner.user.username}` + '>' + ' ]***')
});
